...
describe("OrganizationForm tests", () => {
    ...
    test("renders correctly when passing in initialContents", async () => {
        render(
            <Router>
                <OrganizationForm initialContents={ucsbOrganizationFixtures.oneOrganization[0]} />
            </Router>
        );

        expect(await screen.findByTestId(`${testId}orgCode`)).toBeInTheDocument();
        expect(screen.getByTestId(`${testId}orgCode`)).toHaveValue("1");
        expect(screen.getByTestId(`${testId}-OrgTranslationShort`)).toHaveValue("NAPPING LEAGUE");
        expect(screen.getByTestId(`${testId}-OrgTranslation`)).toHaveValue("Professional Napping League at UCSB");
    ...
    test("No Error messages on good input", async () => {
        ...
        fireEvent.change(orgTranslationShortInput, { target: { value: "PIZZA HUNTERS" } });
        fireEvent.change(orgTranslationInput, { target: { value: "Pizza Hunting Society @ UCSB" } });
        ...
        expect(screen.queryByText(/Max length 10 characters/)).not.toBeInTheDocument();
